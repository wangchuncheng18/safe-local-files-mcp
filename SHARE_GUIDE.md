...
如果朋友的目标是 **GPT 快速聊天读取自己机器的目录**，没有域名就把 [SECURE_TUNNEL.md](SECURE_TUNNEL.md) 发给他；以后要稳定公网地址再看 [QUICK_CHAT_SETUP.md](QUICK_CHAT_SETUP.md)。两份指南都有可粘贴给他本地 Codex 的任务文本。当前先保持只读。
...
- 若要让 GPT 快速聊天访问，无域名路线需要自己的 OpenAI Platform Tunnel、运行密钥和 ChatGPT 开发者模式连接；Cloudflare 稳定地址路线还需要自己控制并已接入 Cloudflare 的域名。
