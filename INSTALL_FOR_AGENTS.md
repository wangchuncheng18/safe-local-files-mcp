...
本文件是一份可执行的安装清单。执行前向用户确认三项实际值：本机操作系统、**本机**授权目录的绝对路径、期望的写权限。默认使用只读。不要把作者电脑的路径复制到另一台机器。
...
1. 仅在用户自己的机器上安装；使用本机 Codex 的 `stdio` MCP。默认不开放 HTTP 端口。
...
3. 新建 Codex 对话，只调用 `stat_path`，参数 `{"path":"."}`。确认返回类型为 `directory`，不列举实际文件。
